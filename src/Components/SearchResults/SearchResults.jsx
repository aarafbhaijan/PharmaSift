...
const SearchResults = ({ data, loading}) => {
  ...
  if (!data.name && loading) {
    return (<>
    <button disabled="" type="button" class="py-2.5 mt-10 px-5 mr-2 text-sm font-medium text-gray-900 bg-white rounded border border-gray-200 hover:bg-gray-100 hover:text-[#0086b6] focus:z-10 focus:ring-4 focus:outline-none focus:ring-[#0086b6] focus:text-[#0086b6]  inline-flex items-center">
      ...
      <>
        <div className="h-3/4 py-6 text-center flex flex-col items-center">
          ...
          <div className=" items-center justify-center flex md:flex-row flex-col gap-6  w-screen">
            <SingleCard data={data.phar1} compare={data}  />

            <SingleCard data={data.phar2} compare={data}  />
          </div>
        </div>
      </>
    )
